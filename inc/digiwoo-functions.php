...
function fetch_products_by_category() {
    ...
    if ($products->have_posts()) {
        while ($products->have_posts()) {
            ...
            $output .= '<label class="col-sm-6 btn"><div class="w-100 btn btn-outline-success py-3 my-3 rounded mx-1 fast-checkout-radio-select fast-checkout-radio-select-product fast-checkout-border-style-1 fast-checkout-title-product text-left"><div class="d-flex justify-content-between lh-condensed"><div><i class="far fa-circle fa-lg mr-2"></i><input type="radio" name="product" value="' . $product_id . '" data-price="' . $product_price . '" disabled>' . $amount. '</div><span class="fast-checkout-box-color-style-2 fast-checkout-text-color-style-1 px-2 py-1 fast-checkout-title-product-price">' .$formatted_price. '</span></div></div>  </label>';
        }
